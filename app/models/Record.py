...
class Record():
    def set_record(self,id_camera, ip, port, type, connect, error, image, plate, status):
        ...
        dic = {'id_comera':id_camera, 'connect':connect,' error':error, 'timestamp':time.time(), "datetime":current_time, 'ip':ip, 'port':port, 'type':type, 'image':image, 'plate':plate, 'status':status}
